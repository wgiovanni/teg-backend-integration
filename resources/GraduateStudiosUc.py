...
class GraduateFaculty(BD, Resource):
    ...
    def get(self):
        try:
            ...
            for row in r:
                item = {
                    "cedula": row['dim_egresado.identificacion'],
...
class GraduatePerYear(BD, Resource):
    ...
    def post(self):
        try:
            ...
            r = browser.aggregate(drilldown=['dim_egresado', 'dim_ano'])
            items = []
            for row in r:
                item = {
                    "cedula": row['dim_egresado.identificacion'],
...
class GraduateFacultyYear(BD, Resource):
    ...
    def post(self):
        try:
            ...
            result = sorted(result, key=lambda k: k['ano']) 
            r = browser.aggregate(drilldown=['dim_egresado', 'dim_ano', 'dim_facultad'])
            items = []
            for row in r:
                item = {
                    "cedula": row['dim_egresado.identificacion'],
